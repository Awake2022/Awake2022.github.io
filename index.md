...
-	7) 스테이지 UI : 화면에 보이는 UI가 적 캐릭터를 가리는 현상이 적었으면 한다.
-	8) 스테이지 UI : UI가 눈에 잘 띄었으면 한다.

<br>

### 4) 그 외

-	9) 스테이지 1의 한 구역이 미완성 상태라도 플레이가 가능했으면 한다.
-	10) 배경 : 움직이는 배경이 있었으면 한다.
-	11) 맵 구성 : 왼쪽에서 오른쪽으로 움직이는 단순한 맵 구성 말고도 다양한 구성의 맵들이 나왔으면 한다. 
