...
-	1. 플레이어 캐릭터의 움직임이 자연스러웠으면 한다. (부동자세의 애니메이션이 2 ~ 3가지 더 있었으면 한다.)
-	2. 플레이어 캐릭터의 조작감이 불편하지 않았으면 한다. (조작키 사용을 멈췄을 때 그 즉시 플레이어의 움직임이 정지했으면 한다.)
-	3. 플레이어 캐릭터의 스킬이 최소 2개에서 4개까지는 사용해보고 싶다.
-	4. 플레이어 캐릭터의 보조 능력이 게임 플레이에 도움이 되는 능력이었으면 한다.

<br>

### 2) 적 캐릭터

- 5. 적 캐릭터들이 최소 2가지 이상의 공격을 했으면 한다.
- 6. 모습이 같은데 색만 다른 적 캐릭터들이 적었으면 한다.
- 7. 보스들의 패턴이 너무 단순하지도 않으면서 부조리하지도 않았으면 한다.

<br>

### 3) 스테이지 구성

-	8. 화면에 보이는 UI가 직관적이고 플레이 중 화면을 가리는 불편함이 적었으면 한다.
-	9. 2D 횡 스크롤 방식이지만 단순 일직선으로 움직여야 되는 방 말고도 다양한 형태의 방이 있으면 한다.
-	10. 미완성되더라도 최소 스테이지 2까지는 플레이 했으면 한다.

<br>

### 4) 그 외

-	11. 무기 선택 창과 스킬 선택 창, 스테이지 선택 창이 동작이라도 했으면 한다.
-	12. 스테이지 1하고 첫 안전 지역의 배경은 나왔으면 한다.
-	13. 게임 구동 시 나오는 화면은 버튼이라도 제대로 작동했으면 한다.
-	14. 게임 처음 시작 시 나오는 스토리는 대화 UI만이라도 나왔으면 한다.
