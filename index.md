...
-	2) 조작 : 조작감이 불편하지 않았으면 한다. (조작키 사용을 멈췄을 때 그 즉시 플레이어의 움직임이 정지했으면 한다.)
-	3) 조작 : 플레이어 스킬을 최소 1개에서 2개, 보조 능력은 최소 1개만이라도 사용했으면 한다.
-	4) 조작 : 플레이어 보조 능력이 게임 플레이에 필요한 능력이었으면 한다. (의미 없는 능력이 아니었으면 한다.)

<br>

### 2) 적 캐릭터

- 5) AI : 적 캐릭터들이 2가지 이상의 공격을 했으면 한다.
- 6) 종류 : 적 캐릭터 2종류 정도가 맵을 돌아다니는 모습을 보고 싶다.
...
-	8) 스테이지 UI : 화면에 보이는 UI가 적 캐릭터를 가리는 현상이 적었으면 한다.
-	9) 스테이지 UI : UI가 눈에 잘 띄었으면 한다.

<br>

### 4) 그 외

-	10) 스테이지 1의 한 구역이 미완성 상태라도 플레이가 가능했으면 한다.
-	11) 배경 : 움직이는 배경이 있었으면 한다.
-	12) 맵 구성 : 왼쪽에서 오른쪽으로 움직이는 단순한 맵 구성 말고도 다양한 구성의 맵들이 나왔으면 한다. 
...
- 1) 플레이어 캐릭터 부동 자세 스프라이트 제작 (부동 자세 1 스프라이트 4개, 부동 자세 2 스프라이트 4개) (50%)
- 2) 플레이어 캐릭터 공격 자세 스프라이트 제작 (공격 자세 1 스프라이트 6개, 공격 자세 2 스프라이트 6개, 공격 자세 3 스프라이트 6개) (0%)
- 3) 적 캐릭터 이동 스프라이트 제작 (적1 이동 스프라이트 8개, 적2 이동 스프라이트 8개) (0%)
- 4) 플레이어 캐릭터 공격 스크립트 제작 (0%)
- 5) 적 캐릭터 이동 스크립트 제작 (0%)
