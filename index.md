...
> ### 컨셉 기반:
> 플레이어가 상대하는 적의 종류, 사용하는 스킬 등으로 얻어지는 카르마 수치로 엔딩이 결정되기에 어떤 스테이지를 선택하고 어떤 스킬을 사용할지 선택하는 재미를 느낄 수 
...
### [만들게 된 배경]  
최근 메트로배니아 장르의 Ori and the Will of the Wisps 게임을 하고 이 장르의 게임들을 알아보다가 Hollow Knight와 ENDER LILIES: Quietus of the Knights라는 게임을 알게 되었다. <br>
그래서 이와 똑같이는 아니더라도 비슷한 게임을 만들어보고 싶었고 기존에 내 상상으로 하던 세계관을 구현해 보자는 생각을 해서 기획하게 되었다.
...
-	플레이어 조작감이 불편하지 않았으면 한다. (조작키 사용을 멈췄을 때 그 즉시 플레이어의 움직임이 정지했으면 한다.)
-	플레이어의 모든 행동들이 자연스러웠으면 한다. (부동자세의 애니메이션이 2~3가지 더 있었으면 한다.)
-	플레이어의 스킬이 최소 2개에서 4개까지는 사용해보고 싶다.
-	플레이어의 보조 능력이 게임 플레이에 도움이 되는 능력이었으면 한다.
-	화면에 보이는 UI가 직관적이고 플레이 중 화면을 가리는 불편함이 적었으면 한다.
-	모습이 같은데 색만 달라졌다고 능력치가 달라지는 Enemy가 적었으면 한다.
-	Enemy들이 최소 2가지 이상의 행동을 했으면 한다.
-	Boss들의 패턴이 너무 단순하지도 않으면서 부조리하지도 않았으면 한다.
-	2D 횡 스크롤 방식이지만 단순 일직선으로 움직여야 되는 방 말고도 다양한 형태의 방이 있으면 한다.
-	미완성되더라도 최소 스테이지 2까지는 플레이 했으면 한다.

<br>

### 2) 그 외

-	무기 선택 창과 스킬 선택 창, 스테이지 선택 창이 동작이라도 했으면 한다.
-	스테이지 1하고 첫 안전 지역의 배경은 나왔으면 한다.
-	게임 구동 시 나오는 화면은 버튼이라도 제대로 작동했으면 한다.
-	게임 처음 시작 시 나오는 스토리는 대화 UI만이라도 나왔으면 한다.
